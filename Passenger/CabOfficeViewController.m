...
@property (weak, nonatomic) IBOutlet UIImageView *secondLogoImageView;
...
- (void)repositionView:(UIView *)view positionDiff:(CGFloat)diff {
    CGRect r = view.frame;
    r.origin.y -= diff;
    view.frame = r;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (IS_NULL(phone) || phone.length == 0)
    {
        ...
        diff += (_phoneButton.frame.origin.y + _phoneButton.frame.size.height) - _phoneNumberTitle.frame.origin.y;
    }
    else
    {
    ...
    if (IS_NULL(email) || email.length == 0)
    {
        ...
        diff += (_emailButton.frame.origin.y + _emailButton.frame.size.height) - _emailButton.frame.origin.y;
    }
    else
    {
        [self repositionView:_emailButton positionDiff:diff];
        [self repositionView:_emailAddressTitle positionDiff:diff];
...
#ifdef SHOW_SECONDARY_CAB_OFFICE_LOGO
    _secondLogoImageView.image = [UIImage imageNamed:@"secondary_logo"];
    [self repositionView:_secondLogoImageView positionDiff:diff];
#else
    _secondLogoImageView.hidden = YES;
#endif
