...
- (void)viewDidLoad
{
    ...
    if ([CabOfficeSettings tosMustAcceptOnSignup])
    {
        ...
        [_tosLabel setLineBreakMode:NSLineBreakByWordWrapping];
        _tosLabel.numberOfLines = 2;
        UITapGestureRecognizer* checkboxTap = [[UITapGestureRecognizer alloc] init];
        [checkboxTap addTarget:self action:@selector(checkboxTapGesture:)];
        checkboxTap.numberOfTapsRequired = 1;
        [_tosLabel setUserInteractionEnabled:YES];
        [_tosLabel addGestureRecognizer:checkboxTap];
...
- (void)updateScrollView {
    ...
    CGPoint p = _activeField.superview.frame.origin;
    p.y += _activeField.superview.frame.size.height;
    if (!CGRectContainsRect(aRect, _activeField.superview.frame) ) {
        CGPoint scrollPoint = CGPointMake(0.0, _activeField.superview.frame.origin.y-_kbSize.height);
        [_topScrollView setContentOffset:scrollPoint animated:YES];
    }
}
...
- (void)keyboardWasShown:(NSNotification*)aNotification
{
    ...
    if (!IS_IPAD) {
        CGSize s = _topScrollView.frame.size;
        s.height += _kbSize.height;
        _topScrollView.contentSize = s;
    }
}
...
{
    if (!IS_IPAD) {
        _topScrollView.contentSize = _topScrollView.frame.size;
    }
...
- (void)checkboxTapGesture:(UITapGestureRecognizer*)gesture {
    _tosCheckMark.checked = !_tosCheckMark.checked;
}
