...
@property (strong, nonatomic) VehiclesManager* vehiclesManager;
@property (strong, nonatomic) SearchContext* searchContext;
...
- (void)viewDidLoad
{
    ...
    _vehiclesManager = [[VehiclesManager alloc] init];
    [_vehiclesManager getVehicles];
    ...
    _bookingActivityIndicator.center = _bookButton.center;
    ...
    self.searchContext = [[SearchContext alloc] init];
    _searchContext.selectedSearchModule = 0;
    _searchContext.searchPickupText = nil;
    _searchContext.searchDropoffText = nil;
}
...
- (void)calculateFare
{
    if (_startAnnotation && _endAnnotation)
    {
        ...
        [[NetworkEngine getInstance] getTravelFare:_startAnnotation.position
                                                to:_endAnnotation.position
                                          usingCar:_vehiclesManager.defaultVehicle.pk
                                    withPickupTime:[[NSDate date] dateByAddingMinimumAllowedPickupTimeOffsetInMinutes]
                                   completionBlock:^(NSObject *o)
                                    {
                                        _bookingActivityIndicator.hidden = YES;
                                        _bookButton.enabled = YES;
                                      ...
                                      failureBlock:^(NSError *e)
                                    {
                                        [MessageDialog showError:e.localizedDescription
...
- (void)finalizeBooking:(NSObject *)response {
...
- (void)updateBooking:(NSString *)pk transaction:(NSDictionary *)transaction amount:(NSNumber *)amount {
    [[NetworkEngine getInstance] updateBooking:pk
                                 transactionId:transaction[@"id"]
                                    amountPaid:amount
                               completionBlock:^(NSObject *response)
                                {
                                    [self finalizeBooking:response];
                                }
                                  failureBlock:^(NSError *error) {
...
- (void)createTransactionForBooking:(NSObject *)response card:(CreditCard *)card {
}

- (void)createBookingOn:(NSDate *)date card:(CreditCard *)card vehicle:(Vehicle *)vehicle {
    NSLog(@"%@", card);
    ...
    [[NetworkEngine getInstance] createBooking:_startAnnotation.title
                               ...
                               completionBlock:^(NSObject *response) {
                                   ...
                                   if (c) {
                                       [self createTransactionForBooking:response card:c];
                                   } else {
                                       [self finalizeBooking:response];
                                   }
                               }
...
#if ENABLE_CREDIT_CARD_PAYMENTS
    if (![CreditCardsManager getInstance].cards.count) {
        [MessageDialog showError:NSLocalizedString(@"new_booking_no_payment_card_defined", @"")
                       withTitle:NSLocalizedString(@"dialog_error_title", @"")];
        return;
    }
#endif
    if ([CabOfficeSettings pickupOrDropoffMustBeStation]) {
        StationsManager* sm = [StationsManager getInstance];
        if (![sm isStation:_startAnnotation.position] && ![sm isStation:_endAnnotation.position]) {
            [MessageDialog showError:NSLocalizedString(@"new_booking_pickup_or_dropoff_not_in_required_area", @"")
                           withTitle:NSLocalizedString(@"dialog_error_title", @"")];
            return;
        }
    }
    
    [ConfirmBookingDialog showDialog:_startAnnotation
                             dropoff:_endAnnotation
                 withVehiclesManager:_vehiclesManager
                   confirmationBlock:^(NSDate *date, Vehicle* vehicle, CreditCard* card){

                       [self createBookingOn:date card:card vehicle:vehicle];
                   }];
}
...
{
    _searchContext.searchPickupText = title;
...
- (void)setDropoffLocation:(CLLocationCoordinate2D)location
                   ...
                   zipCode:(NSString *)zipCode
{
    _searchContext.searchDropoffText = title;
...
- (void)showLocatioSelectionViewController:(LocationType)type sender:(UIView *)sender
{
    ...
    l.searchContext = _searchContext;
    l.locationType = type;
