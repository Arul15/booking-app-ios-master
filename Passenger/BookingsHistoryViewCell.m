...
#ifdef HIDE_PICKUP_AND_DROPOFF_BUTTONS_ON_HISTORY_LIST
    
#define LEFT_MARGIN 40
#define RIGHT_MARGIN 10
    
    _pickupButton.hidden = YES;
    _dropoffButton.hidden = YES;

    CGFloat width = self.frame.size.width - LEFT_MARGIN - RIGHT_MARGIN;
    
    CGRect r = _pickupAndDropoffButton.frame;
    r.origin.x = LEFT_MARGIN;
    r.size.width = width / 2;
    _pickupAndDropoffButton.frame = r;
    
    r = _cancelButton.frame;
    r.origin.x = LEFT_MARGIN + (width / 2);
    r.size.width = width / 2;
