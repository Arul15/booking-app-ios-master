...
//one hour booking cancellation period
#define BOOKING_CANCELLATION_ALLOWED_PERIOD (60*60)

- (BOOL)canCancelBooking:(NSDictionary*)booking {
    BOOL canCancelBooking = NO;
    ...
    if( canCancelBooking ) {
        ...
        NSTimeInterval diff = [[NSDate date] timeIntervalSinceDate:pt];
        if (diff > 0 && diff > BOOKING_CANCELLATION_ALLOWED_PERIOD) {
            canCancelBooking = NO;
        }
    }
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == _selectedRow)
    {
        cell.cancelButton.hidden = ![self canCancelBooking:booking];
    }
    else
    {
    ...
    cell.cancellationBlock = ^(NSInteger index) {
        ...
        NSInteger cancelFeeThreshold = [CabOfficeSettings cancellationFeeTimeThreshold];
        ...
        [MessageDialog askConfirmationWithReason:message
                                       withTitle:NSLocalizedString(@"dialog_confirmation_title", @"")
                               confirmationBlock:^(NSString* message){
                                   [self showPleaseWaitView];
                                   [[NetworkEngine getInstance] cancelBooking:booking[@"pk"]
                                        ...
                                        [_tableView beginUpdates];
                                        [_tableView deleteRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:idx inSection:0]]
                                                          withRowAnimation:UITableViewRowAnimationNone];
                                        [_tableView endUpdates];
