...
{
    _topBarHeight = 32.;
    _topBarBackgroundColor = [UIColor searchDialogBackgroundColor];
    _topBarItemLabelsFont = [UIFont lightOpenSansOfSize:16];
    _pageIndicatorViewSize = CGSizeMake(30., 2.);
}
...
- (void)viewDidLoad
{
    ...
    UIView *s = [[UIView alloc] initWithFrame:CGRectMake(0, self.topBarHeight - 2, 320, 2)];
    s.backgroundColor = [UIColor searchDialogSeparatorColor];
    [self.view addSubview:s];
    
    if (!IS_IPAD)
    {
        UIButton *b = [UIButton buttonWithType:UIButtonTypeCustom];
        [b setImage:[UIImage imageNamed:@"close_button" ] forState:UIControlStateNormal];
        [b setFrame:CGRectMake(4, 4, 24, 24)];
        [b addTarget:self action:@selector(cancelButtonPressed) forControlEvents:UIControlEventTouchUpInside];
...
{
    self.completionBlock(LocationTypePickup, nil);
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [self setSelectedIndex:self.searchContext.selectedSearchModule];
    [self enableScrollViewUserInteraction:YES];
}

- (void)enableScrollViewUserInteraction:(BOOL)enable {
...
{
    self.searchContext.selectedSearchModule = selectedIndex;
    
    UIViewController *pv = _viewControllers[self.selectedIndex];
    UIViewController *nv = _viewControllers[selectedIndex];
    [pv viewDidDisappear:YES];
    [nv viewDidAppear:YES];
    ...
    if (abs(self.selectedIndex - selectedIndex) <= 1) {
        ...
        [UIView animateWithDuration:(animated) ? 0.3 : 0. delay:0. options:UIViewAnimationOptionBeginFromCurrentState animations:^
         {
             [previosSelectdItem setTitleColor:[UIColor searchDialogNotSelectedTitleColor] forState:UIControlStateNormal];
             [nextSelectdItem setTitleColor:[UIColor searchDialogSelectedTitleColor] forState:UIControlStateNormal];
         } completion:nil];
    } else {
...
{
    [self enableScrollViewUserInteraction:NO];
}

#pragma mark - KVO

- (UIColor*) mixColor:(UIColor *)a withColor:(UIColor *)b usingRatio:(CGFloat)ratio
{
    CGFloat ar, ag, ab;
    CGFloat br, bg, bb;
    CGFloat alpha;
    ...
    [a getRed:&ar green:&ag blue:&ab alpha:&alpha];
    [b getRed:&br green:&bg blue:&bb alpha:&alpha];
    
    ar = (ar * ratio) + (br * inv);
    ab = (ab * ratio) + (bg * inv);
    ag = (ag * ratio) + (bb * inv);
    
    return [UIColor colorWithRed:ar green:ag blue:ab alpha:1.0];
}
                       ...
                       context:(void *)context
{
    ...
    if (oldX != self.scrollView.contentOffset.x && self.shouldObserveContentOffset) {
        ...
        if (targetIndex >= 0 && targetIndex < self.viewControllers.count) {
            ...
            UIColor *c1 = [self mixColor:[UIColor searchDialogSelectedTitleColor] withColor:[UIColor searchDialogNotSelectedTitleColor] usingRatio:fabsf(ratio)];
            UIColor* c2 = [self mixColor:[UIColor searchDialogSelectedTitleColor] withColor:[UIColor searchDialogNotSelectedTitleColor] usingRatio:1 - fabsf(ratio)];
            
            [previosSelectedItem setTitleColor:c2 forState:UIControlStateNormal];
            [nextSelectedItem setTitleColor:c1 forState:UIControlStateNormal];
